...
: ${TOOLS_SR_CONNECTION_STR:=$SCHEMA_REGISTRY_PORT_8081_TCP_ADDR:$SCHEMA_REGISTRY_PORT_8081_TCP_PORT}

export TOOLS_ZK_CONNECTION_STR
export TOOLS_KAFKA_CONNECTION_STR
export TOOLS_SR_CONNECTION_STR

if [ "$1" = 'kafka-avro-console-consumer' ]; then
    shift
    exec /usr/bin/kafka-avro-console-consumer \
        --zookeeper ${TOOLS_ZK_CONNECTION_STR} \
        --property schema.registry.url=http://${TOOLS_SR_CONNECTION_STR} \
        "$@"
fi
